...
#
# With this define you can manage any postgresql configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/postgresql/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/postgresql/my.conf',
#
# [*ensure*]
...
define postgresql::conf (
  ...
  $manage_path    = pickx($path, "${postgresql::config_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $postgresql::config_file_mode)
  $manage_owner   = pickx($owner, $postgresql::config_file_owner)
  $manage_group   = pickx($group, $postgresql::config_file_group)
  $manage_require = pickx($config_file_require, $postgresql::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $postgresql::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "postgresql_conf_${name}":
    ensure  => $ensure,
    source  => $source,
